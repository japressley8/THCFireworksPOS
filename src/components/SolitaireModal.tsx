...
export const SolitaireModal: React.FC<SolitaireModalProps> = ({
  isOpen,
  onClose,
  cachedState,
  onSaveCache
  ...
  const [cardColors, setCardColors] = useState({ redSuit: '#e11d48', blackSuit: '#0284c7' });
  const [solidCardBg, setSolidCardBg] = useState('#ffffff');
  ...
  const getColorDistance = (c1: string, c2: string): number => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (fieldEl) {
        ...
        const rawCardBg = computedStyle.getPropertyValue('--color-card').trim();
        
        setSolidCardBg(makeColorOpaque(rawCardBg));

        const dist = getColorDistance(primaryColor, accentColor);
        if (dist < 90) {
          setCardColors({ redSuit: '#e11d48', blackSuit: '#0284c7' });
        } else {
          setCardColors({ redSuit: accentColor, blackSuit: primaryColor });
        }
      }
    }, 150);
  ...
  }, [stock, waste, tableau, foundation, score, history]);
  ...
  const handleUndo = () => {
    ...
    setScore(prev => prev + 1); // Score increases by 1 for undo
  ...
  const moveCards = (source: string, cardIndex: number, target: string) => {
    ...
    if (target.startsWith('foundation-')) {
      ...
      if (movingCards.length === 1) {
        if (fPile.length === 0) {
          isValid = (bottomCard.rank === 1); // Ace only
        } else {
          const topCard = fPile[fPile.length - 1];
          isValid = (bottomCard.suit === topCard.suit && bottomCard.rank === topCard.rank + 1);
        }
      }
      ...
      if (tPile.length === 0) {
        isValid = (bottomCard.rank === 13); // King to empty column
      } else {
  ...
  const handlePointerDown = (e: React.PointerEvent, source: string, cardIndex: number, cardId: string) => {
    // Only drag face-up cards (or recycle clicked separately)
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    // Treat very small pointer offset moves as regular selection clicks
    if (distance < 5) {
    ...
    // Grid Collision checking relative to the board boundaries
    const fieldEl = document.querySelector('.solitaire-field');
    if (fieldEl) {
      ...
      // 1. Foundations: left = 276 + fIdx * 84, top = 20, width = 72, height = 100
      if (relY >= 10 && relY <= 130) {
      ...
      // 2. Tableau columns: left = 24 + pIdx * 84, top = 150, width = 72, height = 330
      if (relY >= 130) {
  ...
  // Perform successful moves and record IDs to disable transition animations
  const executePointerMove = (source: string, cardIndex: number, target: string) => {
    ...
    if (isValid) {
      // Set dropped list to temporarily turn off transitions
      setJustDroppedCardIds(movingIds);
      moveCards(source, cardIndex, target);
    }
  };

  // Clear transition lock state after render
  useEffect(() => {
  ...
  // Selection/Click-to-Move handlers
  ...
  // Double click sends to foundation automatically
  const handleCardDoubleClick = (source: string, cardIndex: number) => {
    ...
    for (let fIdx = 0; fIdx < 4; fIdx++) {
      ...
      if (fPile.length === 0) {
        ...
        const topCard = fPile[fPile.length - 1];
        isValid = (card.suit === topCard.suit && card.rank === topCard.rank + 1);
      }
  ...
  // Coordinates solver for flat list rendering transitions
  ...
  // Helper check for dragging transition override
  ...
  // Auto-complete puzzle solver cascade loop
  useEffect(() => {
    if (!isAutoCompleting) return;

    const makeNextMove = () => {
      // Scan the 7 columns to find a card that can go to foundations
      for (let pIdx = 0; pIdx < 7; pIdx++) {
        const pile = tableau[pIdx];
        ...
        for (let fIdx = 0; fIdx < 4; fIdx++) {
          ...
          if (fPile.length === 0) {
            ...
            canMove = (card.suit === fTop.suit && card.rank === fTop.rank + 1);
          }
  ...
  // Sort by card.id to keep DOM ordering completely static (guarantees browser transitions trigger reliably)
  ...
  return (
    <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-5xl bg-custom-card border border-custom-border rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col h-[90vh] max-h-[750px] relative">
        ...
        <div className="bg-custom-header border-b border-custom-border px-6 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <h3 className="font-black text-custom-text text-base flex items-center gap-3 tracking-wide uppercase select-none">
              ...
              <span>Solitaire Fireworks Show</span>
        ...
        <div className="flex-1 flex overflow-hidden min-h-0">
          ...
          <div className="flex-1 p-6 overflow-y-auto flex flex-col justify-between select-none">
            <div>
              ...
              >
                ...
                {/* Moves Stats badge removed - now in header */}

                {/* 4 Foundations empty placeholders */}
                {Array.from({ length: 4 }).map((_, fIdx) => {
                  const sourceName = `foundation-${fIdx}`;
                  const suitsList: ('hearts' | 'diamonds' | 'clubs' | 'spades')[] = ['hearts', 'diamonds', 'clubs', 'spades'];
                  const pileSuit = suitsList[fIdx];
                ...
                {allCardsToRender.map(({ card, source, index }) => {
                  ...
                  // Reverse stagger delay: right-most card (drawnOffset = 2) flies first with 0ms, left-most (drawnOffset = 0) flies last with 300ms.
                  ...
                  return (
                    ...
                    >
                      {/* CARD FRONT SIDE (Visible only when face up) */}
                      <div
                        className={`w-full h-full rounded-lg border flex flex-col justify-between p-1.5 transition-all duration-200 select-none shadow-md absolute inset-0 ${
                          isSelected 
                        ...
                        style={{
                          ...
                          color: card.color === 'red' ? cardColors.redSuit : cardColors.blackSuit,
                      ...
                      {/* CARD BACK SIDE (Visible only when face down) */}
                      <div
              ...
              <div className="flex justify-center gap-4 mt-4 shrink-0">
                <button
                  ...
                  className="px-6 py-2 bg-custom-input hover:bg-custom-primary/20 text-custom-muted hover:text-custom-text disabled:opacity-40 rounded-xl transition-all border border-custom-border flex items-center gap-2 text-xs font-bold shadow-md"
                ...
                <button
                  ...
                  className="px-6 py-2 bg-custom-input hover:bg-custom-primary/20 text-custom-muted hover:text-custom-text rounded-xl transition-all border border-custom-border flex items-center gap-2 text-xs font-bold shadow-md"
                  title="Restart Game"
                >
